...
//        multimeterInstrument = rm.makeInstrument(named: multimeterUII) as? MessageBasedInstrument
    ...
    func testSetWaveformCharacteristics() {
        ...
        do {
            ...
            try waveformGeneratorInstrument?.write(command: "SOURCE1:FUNCTION DC")
            try waveformGeneratorInstrument?.write(command: "SOURCE1:VOLTAGE 5");

            // Turn the output on
            try waveformGeneratorInstrument?.write(command: "OUTPUT1 ON")
        } catch {
            XCTFail()
        }
    }
    ...
    func testReadDCVoltage() {
        ...
        do {
            try multimeterInstrument?.write(command: "MEASURE:VOLTAGE:DC?")
            let voltage = try multimeterInstrument?.read()
    ...
    func testQuery() {
        ...
        do {
            let voltage = try multimeterInstrument?.query(command: "MEASURE:VOLTAGE:DC?")
            XCTAssertNotNil(voltage)
        } catch {
            XCTFail()
        }
